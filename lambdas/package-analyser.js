...
const getPackageStat = async (name, version) => {
  if (name && version) {
    const cachedStats = await dynamodb.retrieveStat(name, version)
    if (cachedStats) {
      return cachedStats
    }
  }
  ...
  const cachedStatsAfterYarn = await dynamodb.retrieveStat(info.name, info.version)
  if (cachedStatsAfterYarn) {
    return cachedStatsAfterYarn
  }
